...
--		FROM DUAL WHERE NOT EXISTS(SELECT 1 FROM user_material WHERE material_name = '按钮' AND img_key = 'group_button' AND terminal_type = 2);
		...
		INSERT INTO user_material (material_name, material_state, createtime, terminal_type, user_id, img_key) 
		SELECT '常规', '0',now(),1,0,'controls_rectangle' 
		FROM DUAL WHERE NOT EXISTS(SELECT 1 FROM user_material WHERE material_name = '常规' AND img_key = 'controls_rectangle' AND terminal_type = 1);
		INSERT INTO user_material (material_name, material_state, createtime, terminal_type, user_id, img_key) 
		SELECT '常规', '0',now(),2,0,'controls_rectangle'
		FROM DUAL WHERE NOT EXISTS(SELECT 1 FROM user_material WHERE material_name = '常规' AND img_key = 'controls_rectangle' AND terminal_type = 2);
...
IF  EXISTS(SELECT * FROM information_schema.`COLUMNS` WHERE COLUMN_NAME="order_level" AND TABLE_NAME=tn AND TABLE_SCHEMA=ts) THEN
		...
		update user_material set order_level = 1  where material_id in (select material_id  from (SELECT material_id FROM user_material WHERE img_key = 'controls_rectangle') a );
		
		 update user_material set order_level = 2  where material_id in (select material_id  from (SELECT material_id FROM user_material WHERE img_key = 'controls_text') a);
  	END if;
		
	alter table user_material modify column material_id varchar(255);	
	UPDATE user_material SET material_id = concat('mtr' , material_id) WHERE material_id NOT LIKE 'mtr%';
	IF NOT EXISTS (select * from information_schema.statistics where table_schema=ts and TABLE_NAME=tn and COLUMN_NAME='material_id' and index_name !='primary') then
	 		ALTER TABLE `user_material` ADD INDEX user_material_material_id ( `material_id` );
	END IF;
 END IF;
DESC user_material;
END;
