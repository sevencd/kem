...
if not exists(select id from key_config where prefix="ctg") then
	INSERT INTO `key_config` 
	(prefix,seed,is_enable,add_time,update_time,len,step)
	VALUES ('ctg', 1, 1, now(), now(), 12, 50);
end if;
if not exists(select id from key_config where prefix="customer") then
	INSERT INTO `key_config` 
	(prefix,seed,is_enable,add_time,update_time,len,step)
	VALUES ('customer', 1, 1, now(), now(), 12, 50);
end if;
if not exists(select id from key_config where prefix="odr") then
	INSERT INTO `key_config` 
	(prefix,seed,is_enable,add_time,update_time,len,step)
	VALUES ('odr', 1, 1, now(), now(), 12, 50);
end if;
