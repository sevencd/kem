...
public class PluginDao extends BaseDao {
	...
	private boolean isExists(ActivePluginEntity entity) throws DaoAppException {
		String sql = "SELECT upi.plugin_id FROM user_plugin_info as upi left join user_plugin_activeplugin as upa on upi.plugin_id = upa.plugin_id  WHERE upi.relation_id=? AND upi.relation_type=? AND upi.plugin_type=2 AND upa.type=?;";
		Integer val = -1;
	...
	private boolean isExists(PluginEntity entity) throws DaoAppException {
		String sql = "SELECT plugin_id FROM user_plugin_info WHERE relation_id=? AND relation_type=? AND plugin_type=1;";
		Integer val = -1;
		List<Object> paramter = null;
		paramter = new ArrayList<Object>();
		paramter.add(entity.getRelationId());
	...
	private void disablePlugin(String relationId) throws DaoAppException {
		final String sqlScript = "UPDATE user_plugin_info SET is_used=0 WHERE relation_id=?;";
		List<Object> paramter = null;
		paramter = new ArrayList<Object>();
	...
	private int add(final PluginEntity entity) throws DaoAppException {
		final String sql = "INSERT INTO user_plugin_info (plugin_type,create_time,user_id,relation_id,relation_type,is_used)VALUES(?,?,?,?,?,?);";
		int val = -1;
	...
	private Entity query(QueryPlugin entity) throws DaoAppException {
		String sql = "SELECT * FROM user_plugin_info  WHERE relation_id=? AND plugin_type=?";
	...
	private Entity query(ActivePluginEntity entity) throws DaoAppException {
		String sql = "SELECT * FROM user_plugin_info as upi left join user_plugin_activeplugin as upa on upi.plugin_id = upa.plugin_id  WHERE upi.relation_id=? AND upi.plugin_type=2 AND upa.type=?";
		List<Object> para = null;
		SqlRowSet rs = null;
		para = new ArrayList<Object>();
		para.add(entity.getRelationId());
