...
@Component("activepluginDao")
public class ActivePluginDao extends BaseDao {

	public ActivePluginDao() {
		super();
	...
	private int deleteDrawprizeData(DeleteDrawprizeDataDto deleteDrawprizeDataDto) throws DaoAppException {
		StringBuilder sql = new StringBuilder();
		sql.append("delete  from  user_plugin_activeplugin_drawprizedata where record_id=?");
		return this.execUpdate(sql.toString(), deleteDrawprizeDataDto.getRecordId());
	}
	...
	private int updatePrizeSetting(ActivePluginPrizeSettingEntity activePluginPrizeSettingEntity)
			throws DaoAppException {
		String sql = " UPDATE  user_plugin_activeplugin_prizesetting set amount = amount - 1 where record_id = "
	...
	private int update(DrawPrizeRecordEntity entity) throws DaoAppException {
		...
		return this.execUpdate(sql.toString(), entity.getPluginId(), entity.getPhoneNo(), entity.getName(),
				entity.getAddress(), entity.getQq(), entity.getCreatetime(), entity.getPrizeName(), entity.getPrizeNo(),
				entity.getExchangeState(), entity.getExchangeTime(), entity.getRecordId());
	}

	private int add(final DrawPrizeRecordEntity entity) throws DaoAppException {

		final String sql = "insert into user_plugin_activeplugin_drawprizedata (plugin_id,phone_no,name,address,qq,createtime,prize_name,prize_no,exchange_state) values (?,?,?,?,?,?,?,?,?)";
	...
	private boolean isExists(DrawPrizeRecordEntity entity) throws DaoAppException {
		if (entity.getRecordId() == null)
			return false;
		String sql = "SELECT record_id FROM user_plugin_activeplugin_drawprizedata WHERE record_id=?;";
		Integer val = -1;
	...
	public Entity query(Entity entity, boolean flag) throws DaoAppException {
		if (entity instanceof QueryActivePluginByRelationId) {
			...
			return queryDrawPrizeRecordByRecordId(((QueryDrawPrizeRecordByRecordIdDto) entity).getRecordId());
	...
	private Iterator<Entity> queryUserDrawPrizeCount(QueryDrawprizeDataDto queryDrawprizeDataDto)
			throws DaoAppException {
		...
		SqlRowSet row = this.execQueryForRowSet(builder.toString(), args.toArray());
	...
	private Iterator<Entity> queryUserDrawPrize(QueryDrawprizeUserDataDto queryDrawprizeDataDto)
			throws DaoAppException {
		StringBuilder builder = new StringBuilder(
	...
	private Entity queryUserDrawPrizeForCount(QueryDrawprizeUserDataDto queryDrawprizeDataDto) throws DaoAppException {
		StringBuilder builder = new StringBuilder(
	 ...
	 * @throws DaoAppException
	 */
	private DrawPrizeRecordEntity queryDrawPrizeRecordByRecordId(Integer recordId) throws DaoAppException {
		StringBuilder builder = new StringBuilder();
		...
		// List<Object> args=new ArrayList<Object>();
		// args.add(recordId);
		SqlRowSet row = this.execQueryForRowSet(builder.toString(), recordId);
	...
	private ActivePluginEntity queryActivePluginByRelationId(final QueryActivePluginByRelationId condition)
			throws DaoAppException {
		StringBuilder builder = new StringBuilder();
		List<Object> args = new ArrayList<Object>();
		...
		builder.append(" and upi.relation_id=? and upi.plugin_type =" + PluginType.ACTIVEPLUGIN.getValue());
		...
		entity.setRelationType(ManuscriptType.getEnum(row.getInt("relation_type")));
		...
		entity.setPrizeCollectInfo(
				FastJson.json2Bean(row.getString("prize_collect_info"), entity.getPrizeCollectInfo().getClass()));
		entity.setPrizeCollectRequiredInfo(FastJson.json2Bean(row.getString("prize_collect_required_info"),
		...
		String recordSql = "select record_id,option_name,prize_name,amount,rate from user_plugin_activeplugin_prizesetting where plugin_id=?;";
		row = this.execQueryForRowSet(recordSql, entity.getPluginId());

		List<ActivePluginPrizeSettingEntity> settings = new ArrayList<ActivePluginPrizeSettingEntity>();
		while (row.next()) {
			ActivePluginPrizeSettingEntity record = new ActivePluginPrizeSettingEntity(entity.getPluginId());
	...
	private int add(final ActivePluginEntity entity) throws DaoAppException {
		// 插入插件表
		final String sqlScript = "insert into user_plugin_info (plugin_type,create_time,user_id,relation_id,relation_type,is_used) values (?,?,?,?,?,?);";
		JdbcTemplate template = null;
		KeyHolder holder = null;
		int val = -1;
		if (entity == null)
		...
		val = template.update(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				statement.setObject(2, new Timestamp(entity.getCreatetime().getTime()));
		...
		// 插入活动插件表
		final String sqlScriptAcitve = "insert into user_plugin_activeplugin (plugin_id,type,drawtime,wintime,intervaltime,intervaltime_type,prize_collect_info,prize_collect_required_info,outer_url,merchant_phone) values (?,?,?,?,?,?,?,?,?,?);";
		...
		val = template.update(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				if (entity.getActivePluginType() == null) {
					...
					statement.setInt(2, entity.getActivePluginType().getValue());
				}
				...
				statement.setString(7, FastJson.bean2Json(entity.getPrizeCollectInfo()));
				statement.setString(8, FastJson.bean2Json(entity.getPrizeCollectRequiredInfo()));
	...
	private int update(final ActivePluginEntity entity) throws DaoAppException {
		// 修改插件表
		final String sqlScript = "UPDATE user_plugin_info SET plugin_type=?,create_time=?,user_id=?,relation_id=?,relation_type=?,is_used=? WHERE plugin_id=?;";
		JdbcTemplate template = null;
		int val = -1;
		if (entity == null)
			return val;
		...
		// 修改活动插件表
		final String sqlScriptActive = "UPDATE user_plugin_activeplugin SET drawtime=?,wintime=?,intervaltime=?,intervaltime_type=?,prize_collect_info=?,prize_collect_required_info=?,outer_url=?,merchant_phone=? WHERE plugin_id=?;";
		...
		val = template.update(sqlScriptActive, new PreparedStatementSetter() {
			public void setValues(PreparedStatement statement) throws SQLException {
				...
				statement.setString(5, FastJson.bean2Json(entity.getPrizeCollectInfo()));
				statement.setString(6, FastJson.bean2Json(entity.getPrizeCollectRequiredInfo()));
	 ...
	 * @throws DaoAppException
	 */
	private boolean isExists(final ActivePluginEntity entity) throws DaoAppException {
		final String sqlScript = "SELECT plugin_id FROM user_plugin_info WHERE plugin_id=?";
		JdbcTemplate template = null;
		Integer val = -1;
		if (entity == null)
	...
	private void insertPrizeSettings(final ActivePluginEntity pluginEntity) {
		...
		List<ActivePluginPrizeSettingEntity> entities = pluginEntity.getActivePluginPrizeSettings();
		if (entities == null || entities.isEmpty()) {
			return;
		}

		// 删除原有数据
		final String delSqlScript = "DELETE FROM user_plugin_activeplugin_prizesetting WHERE plugin_id=?;";
		JdbcTemplate template = null;
		int val = -1;
		...
		for (final ActivePluginPrizeSettingEntity entity : entities) {
			...
			final String sqlScript = "insert into user_plugin_activeplugin_prizesetting (plugin_id,option_name,prize_name,amount,rate) values (?,?,?,?,?);";
			KeyHolder holder = null;
			val = -1;
