...
import cn.ilanhai.kem.mail.protocol.sohu.address.QueryAddressListResponse;
import cn.ilanhai.kem.mail.sohu.SohuMailConfig;
import cn.ilanhai.kem.util.HttpHelper;

public class QueryAddressList {
	public QueryAddressList() {

	}

	public QueryAddressListResponse query(SohuMailConfig cfg,
			QueryAddressListRequest req) throws AppException {
		String doc = null;
		Map<String, String> params = null;
		QueryAddressListResponse resp = null;
		if (cfg == null)
		...
		if (req.getAddress() == null || req.getAddress().size() <= 0)
		...
		resp = Result.jsonToObject(doc, QueryAddressListResponse.class);
		if (resp == null)
	 ...
	 * 日志
	 */
	private final Logger LOGGER = Logger.getLogger(QueryAddressList.class);
	/**
	 * 请求的url
	 */
	private final String URL = "http://api.sendcloud.net/apiv2/addresslist/list";
}
